...
def extract_question_set_id_from_zip(zip_file):
    """
    Extract the question_set_id from the question_sets_questions.json inside the ZIP file.
    """
    with zipfile.ZipFile(zip_file, 'r') as zip_ref:
        for file_name in zip_ref.namelist():
            if 'question_sets_questions.json' in file_name:
                with zip_ref.open(file_name) as json_file:
                    data = json.load(json_file)
                    if "question_set_id" in data:
                        return data["question_set_id"]
                    # Assuming there could be multiple entries; use the first one.
                    return data[0].get("question_set_id")
...
def html_upload_and_prepare(request):
    progress_steps = []
    ...
    if request.method == 'POST':
        ...
        # Step 2: Extract `resource_id` from `question_sets_questions.json` inside the zip
        resource_id = extract_question_set_id_from_zip(zip_file)
        if not resource_id:
            progress_steps.append("Failed to extract `resource_id` from the ZIP file.")
            return HttpResponse("Failed to extract `resource_id` from the ZIP file.", status=500)
        progress_steps.append(f"Extracted resource ID: {resource_id}")

        set_aws_credentials()
        # Step 3: Save the uploaded ZIP file to a temporary location and upload it to S3
        with tempfile.NamedTemporaryFile(delete=False, suffix='.zip') as temp_zip:
            for chunk in zip_file.chunks():
                temp_zip.write(chunk)
            temp_zip_path = temp_zip.name

        s3_file_url = upload_to_s3(temp_zip_path)
        os.remove(temp_zip_path)  # Clean up the temporary file
        progress_steps.append("ZIP file uploaded to S3.")
    ...
    return render(request, 'htmlcoding/upload_form.html')
